...
def	transformImage(image, originalMesh, transformedMesh, triangles):
	...
	for triangle in triangles:
		...
		if signedArea > 0:
			...
			for row in range(minRow, minRow + maxRow - minRow):
				for col in range(minCol, minCol + maxCol - minCol):
...
def main():
	...
	transformedSource = transformImage(source, sourceMesh, transformedMesh, sourceTriangles)
